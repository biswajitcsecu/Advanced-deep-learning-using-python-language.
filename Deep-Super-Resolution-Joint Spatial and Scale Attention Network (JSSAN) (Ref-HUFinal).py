...
def conv_block(inputs, filters, kernel_size):
    ...
    x = Activation('relu')(x)
    x = Conv2D(filters, kernel_size, padding='same', kernel_initializer=RandomNormal(stddev=0.01))(x)
    x = Activation('relu')(x)   
...
def jssan(input_shape):
    ...
    # Upsampling block
    x = conv_block(concatenated_features, 32, 3)
    x = conv_block(x, 64, 3)
    x = conv_block(x, 128, 3)
    x = conv_block(x, 256, 3)
    x = conv_block(x, 512, 3)
    
    x = UpSampling2D(size=1)(x)
...
for i in tqdm(range(20)):
    ...
    himage = x_test[idx]
    limage = y_test[idx]
    pred_image = model.predict(tf.expand_dims(image,axis=0))[0]
        
    plt.figure(figsize=(10,8))
    plt.subplot(1,4,1)
    show_image(himage, title="Higher Image")
        
    plt.subplot(1,4,2)
    show_image(limage, title="Lower Image")
        
    plt.subplot(1,4,3)
    show_image(pred_image, title="Predicted Image")  
