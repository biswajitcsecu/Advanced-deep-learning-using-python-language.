...
history = model.fit(x_train, y_train, epochs=nepochs, validation_data=(x_test, y_test),
                    ...
                    max_queue_size=2,workers=4,use_multiprocessing=True,
                   )
#train_generator
...
#Display
print("Input -----------Ground Truth------------Predicted Outcome")
for i in tqdm(range(6)):    
    ...
    ax.axis('off')
    ax.imshow(np.clip(x, 0, 1)) #Img = np.clip(Img, 0, 1) 

    ax = fig.add_subplot(1, 3, 2)
    ax.axis('off')
    ax.imshow(np.clip(y, 0, 1))
...
for i in  tqdm(range(0,9,3)):
    ...
    plt.xticks([])
    plt.yticks([])
    plt.imshow( np.clip(x_test[i], 0, 1)) #Img = np.clip(Img, 0, 1) 
...
for i in tqdm(range(20)):
    ...
    plt.subplot(1,4,1)
    show_image(image , title="Higher Image") #Img = np.clip(Img, 0, 1) 
...
def plot_images(high,low,predicted):
    ...
    plt.axis('off')
    plt.imshow(high) #Img = np.clip(Img, 0, 1) 
    plt.subplot(1,3,2)
...
plt.imshow(np.clip(img, 0, 1) ) #Img = np.clip(Img, 0, 1) 
plt.axis('off')
...
#Activation silency map
img_path="ISRU/img2.png"
image = load_img(img_path, target_size= (H, W),color_mode='rgb')
...
#layers
layers=['conv2d','conv2d_2','conv2d_3','conv2d_4','conv2d_5']   
...
#Display distribution of each layer
